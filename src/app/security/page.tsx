...
export default function SecurityPage() {
    ...
    useEffect(() => {
        async function fetchSecurityData() {
            try {
                const [membersResp, logsResp] = await Promise.all([
                    fetch('/api/members'),
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="glass-card flex items-center gap-5 border-l-4 border-blue-500">
                    ...
                    <div>
                        ...
                        <h3 className="text-2xl font-black text-white italic tracking-tight">ENFORCED</h3>
                    </div>
                </div>
                <div className="glass-card flex items-center gap-5 border-l-4 border-purple-500">
                    ...
                    <div>
                        ...
                        <h3 className="text-2xl font-black text-white italic tracking-tight">{members.length > 0 ? members.length * 2 : 12} ACTIVE</h3>
                    </div>
                </div>
            </div>
                    ...
                    <div className="glass-card !p-0 overflow-hidden">
                        <div className="p-4 bg-white/5 border-b border-white/5 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] bg-white/10 px-2 py-0.5 rounded-full text-muted-foreground">{members.length}</span>
                            </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="glass-card space-y-6 relative overflow-hidden">
                        ...
                        <div className="space-y-6 relative z-10">
                            {logs.length === 0 ? (
                                <div className="py-8 text-center text-muted-foreground text-xs font-medium">No audit events recorded.</div>
                            ) : logs.map((log, i) => (
                                <div key={i} className="flex gap-4">
                                    <div className={cn(
                                        "mt-1 p-2 rounded-lg bg-black/40 border border-white/5",
                                        log.level === 'error' ? 'text-rose-400' :
                                            log.level === 'warn' ? 'text-yellow-400' : 'text-purple-400'
                                    )}>
                                        {log.level === 'error' ? <ShieldAlert className="w-4 h-4" /> : <ShieldCheck className="w-4 h-4" />}
                                    </div>
                                    <div>
                                        <p className="text-sm font-bold text-white py-0.5">{log.message}</p>
                                        <div className="flex items-center gap-2 text-[10px] text-muted-foreground font-medium">
                                            <span className="text-white/80">{log.details?.user || 'System'}</span>
