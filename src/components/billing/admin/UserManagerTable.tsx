...
interface UserData {
    ...
    updated_at: string;
    user_usage_quotas: { token_usage_current: number; token_usage_limit: number }[];
}

interface UserManagerTableProps {
    users: UserData[];
    onUpdateTier: (userId: string, newTier: string) => Promise<void>;
}

export function UserManagerTable({ users, onUpdateTier }: UserManagerTableProps) {
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="w-24">
                                            <div className="flex justify-between text-[10px] text-slate-500 mb-1">
                                                <span>{(user.user_usage_quotas?.[0]?.token_usage_current || 0).toLocaleString()}</span>
                                                <span>/</span>
                                                <span>{user.user_usage_quotas?.[0]?.token_usage_limit === 999999999 ? '∞' : (user.user_usage_quotas?.[0]?.token_usage_limit || 0).toLocaleString()}</span>
                                            </div>
                                            <div className="h-1 bg-slate-800 rounded-full overflow-hidden">
                                                <div
                                                    className="h-full bg-indigo-500"
                                                    style={{
                                                        width: `${Math.min(100, ((user.user_usage_quotas?.[0]?.token_usage_current || 0) / (user.user_usage_quotas?.[0]?.token_usage_limit || 1)) * 100)}%`
                                                    }}
                                                />
                                            </div>
                                        </div>
                                            ...
                                            {['free', 'pro', 'enterprise'].filter(t => t !== user.tier).map(tier => (
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        onUpdateTier(user.user_id, tier).finally(() => setUpdating(null));
                                                    }}
